...
- **`read_file`**: Base = "Read the content of a single file. Use this in preference to execute_command(\"cat ...\"): no shell, structured truncation. Use encoding='base64' for binary files."
...
- **`write_file`**: Base = "Write content to a file (creates or overwrites). Use encoding='base64' for binary; set create_dirs=true to mkdir -p the parent."
...
- **`list_directory`**: Base = "List a directory's entries. Prefer this over execute_command(\"ls ...\"): no shell, structured d/f/l prefixes. With no path it lists the server working directory."
