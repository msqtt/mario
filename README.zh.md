...
- 🎚 **MODE 控制** — `MODE=read|write|yolo` 控制审批行为：`read`（默认）cwd 内写需审批；`write` cwd 内读写自由；`yolo` 跳过所有路径审批（硬编码安全封锁仍然生效）
...
| `MODE` | `read` | 审批模式：`read`（cwd 内写需审批）、`write`（cwd 内读写自由，cwd 外需审批）、`yolo`（跳过所有路径审批；硬编码安全封锁仍然生效） |
...
| MODE | cwd 内读 | cwd 内写 | cwd 外访问 |
|------|----------|----------|-----------|
| `read`（默认） | ✅ 自由 | ⚠️ 需审批 | ⚠️ 需审批 |
| `write` | ✅ 自由 | ✅ 自由 | ⚠️ 需审批 |
| `yolo` | ✅ 自由 | ✅ 自由 | ✅ 自由 |

所有模式下，`write_file` 写入 cwd **外部**路径都需审批（`yolo` 除外）。硬编码封锁（第一层）**永远不会**被任何模式绕过。
